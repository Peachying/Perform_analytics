...
def deep_CNN(images, batch_size, n_classes):
    tf.summary.image('input', images, batch_size)
    # 搭建网络
    # 第一层卷积
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        tf.summary.histogram(scope.name, h_conv1)
    # 第一层池化
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        tf.summary.histogram(scope.name, h_conv2)
    # 第二层池化
    # 3x3最大池化，步长strides为2，池化后执行lrn()操作
    with tf.variable_scope('pooling2_lrn') as scope:
        pool2 = max_pool_2x2(h_conv2, 'pooling2')  # 得到32*32*32
        norm2 = tf.nn.lrn(pool2, depth_radius=4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm2')
        tf.summary.histogram(scope.name, norm2)
    # 第三层卷积
    ...
    with tf.variable_scope('conv3') as scope:
        ...
        tf.summary.histogram(scope.name, h_conv3)
    # 第三层池化
    # 3x3最大池化，步长strides为2，池化后执行lrn()操作
    with tf.variable_scope('pooling3_lrn') as scope:
        ...
        tf.summary.histogram(scope.name, norm3)
    # 第四层全连接层
    ...
    with tf.variable_scope('local3') as scope:
        ...
        tf.summary.histogram(scope.name, h_fc1)
    # 第五层全连接层
    # 128个神经元，激活函数relu()
    with tf.variable_scope('local4') as scope:
        ...
        tf.summary.histogram(scope.name, h_fc2)
    ...
    with tf.variable_scope('softmax_linear') as scope:
        ...
        tf.summary.histogram(scope.name, softmax_linear)
