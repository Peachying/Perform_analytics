...
def get_file(file_dir):
    ...
    for file in os.listdir(file_dir + '/0'):
        normal.append(file_dir + '/0' + '/' + file)
        label_normal.append(0)
    for file in os.listdir(file_dir + '/1'):
        abnormal.append(file_dir + '/1' + '/' + file)
        label_abnormal.append(1)
    
 
    # 打印出提取图片的情况，检测是否正确提取
    print("There are %d normal\nThere are %d abnormal" %(len(normal), len(abnormal)),end="")
    ...
    image_list = np.hstack((normal, abnormal))
    label_list = np.hstack((label_normal, label_abnormal))
    # 利用shuffle，转置、随机打乱
...
def get_batch(image, label, image_W, image_H, batch_size, capacity):
    ...
    image = tf.decode_raw(image_contents, tf.uint8)
    # jpeg或者jpg格式都用decode_jpeg函数，其他格式可以去查看官方文档
 
    # step3：数据预处理，对图像进行旋转、缩放、裁剪、归一化等操作，让计算出的模型更健壮。
    image_bytes = image_W * image_H *3
    image = tf.reshape(tf.slice(image, [0], [image_bytes]), [image_W, image_H, 3])
